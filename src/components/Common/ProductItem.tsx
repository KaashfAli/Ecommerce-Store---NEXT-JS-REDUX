...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  const handleItemToWishList = () => {
    if (item?.quantity === 0) {
      ...
      toast.error("Only out-of-stock items can be added to wishlist");
    }
  };
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-[#F6F7FB] min-h-[270px] mb-4">
        {item.quantity === 0 && (
          <span className="absolute top-3 left-3 z-10 inline-flex items-center rounded-md bg-red-600 px-2 py-0.5 text-xs font-semibold shadow">
            Out of Stock
          </span>
        )}
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          ...
          <button
            ...
            className={`inline-flex font-medium text-custom-sm py-[7px] px-5 rounded-[5px] ease-out duration-200 ${item.quantity === 0
          ...
          <button
            ...
            id="favOne"
            disabled={item.quantity !== 0}
            title={item.quantity !== 0 ? "Only out-of-stock items can be wishlisted" : "Add to wishlist"}
            className={`flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 ${item.quantity === 0
              ? "text-dark bg-white hover:text-blue cursor-pointer"
      ...
      <div className="flex items-center gap-2.5 mb-2">
        ...
        <p className="text-custom-sm">({item.quantity})</p>
      </div>
      ...
      <span className="flex items-center gap-2 font-medium text-lg">
        <span className="text-dark">{item.discountedPrice}Rs</span>
        <span className="text-dark-4 line-through">{item.price}Rs</span>
      </span>
    </div>
  );
};
