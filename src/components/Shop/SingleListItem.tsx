...
const SingleListItem = ({ item }: { item: Product }) => {
  ...
  const handleItemToWishList = () => {
    if (item?.quantity === 0) {
      ...
      toast.error("Only items with 0 quantity can be added to wishlist");
    }
  };
    ...
    <div className="group rounded-lg bg-white shadow-1">
      <div className="flex">
        <div className="shadow-list relative overflow-hidden flex items-center justify-center max-w-[270px] w-full sm:min-h-[270px] p-4">
          {item.quantity === 0 && (
            <span className="absolute top-3 left-3 z-10 inline-flex items-center rounded-md bg-red-600 px-2 py-0.5 text-xs font-semibold text-white shadow">
              Out of Stock
            </span>
          )}
